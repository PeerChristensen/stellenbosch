...
#' geom_point(alpha=.3) +
#' scale_colour_stellenbosch(discrete=F,"main")
#'
#' ggplot(diamonds,aes(x=carat,y=price, colour= price)) +
#' geom_point(alpha=.3) +
#' scale_colour_stellenbosch(discrete=F,"wine")
#'
#' colorblindness test
...
stellenbosch_cols <- function(...) {
  ...
  if (is.null(cols))
    return (stellenbosch_colours)

  stellenbosch_colours[cols]
}
